        ...
        return new Promise((resolve, reject) => {
            ...
            data['dbHost'].forEach((dbHost, i) => {
                ...
                if (type === 'oracle') {
                    ...
                    parameter = [database, user, ip, database, user, ip];
                } else if (type === 'postgres') {
                    parameter = [database, user, ip];
                }

                // 우선 해당 호스트는 제외
                // DB커넥션 개수 가져오는 쿼리 실행
                dbconn.execQuery(pools[dbHost], sql, parameter, type).then(function (result) {
                    let saveDataFormat = {
                        act_cnt: 0,
                        in_act_cnt: 0,
                        tot_conn_cnt: 0,
